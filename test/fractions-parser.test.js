...
import Parser from '../src/fractions-parser.js';

describe('Parser:', () => {
  describe('parse ast', () => {
    const parse = (e) => Parser.parse(e).ast;
  ...
  describe('calc', () => {
    const calc = (e) => Parser.parse(e).calc();
    const calc_s = (e) => calc(e).toString();
    ...
    it('missing term gives error', () => {
      const message = 'incomplete expression';
  ...
  describe('render', () => {
    const render = (e, o) => Parser.parse(e).render(o);
    ...
    it('with result, simple and mixed fractions', () => {
      expect(render('1/8+2/8', { result: true })).toBe('\\frac{1}{8} + \\frac{2}{8} = \\frac{3}{8}');
      expect(render('7/8+2/8', { result: true })).toBe('\\frac{7}{8} + \\frac{2}{8} = \\frac{9}{8} = 1 \\frac{1}{8}');
      expect(render('9/8', { result: true })).toBe('\\frac{9}{8} = 1 \\frac{1}{8}');
      expect(render('1/8', { result: true })).toBe('\\frac{1}{8}');
      expect(render('4/8', { result: true })).toBe('\\frac{4}{8} = \\frac{1}{2}');
      expect(render('1 1/8', { result: true })).toBe('1 \\frac{1}{8} = \\frac{9}{8}');
    });

    it('render with result, errors', () => {
      expect(render('4/0', { result: true })).toEqual({ error: 'Division by zero!' });
      expect(render('4/', { result: true })).toEqual({ error: 'incomplete expression' });
    });
  });
});
